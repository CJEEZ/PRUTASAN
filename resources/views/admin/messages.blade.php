...
@section('content')
@php
    $adminName = auth()->user()->name ?? 'Admin Panel';
    ...
    // Keep the admin inbox isolated to its own role group while still allowing the admin to manage all user-role conversations.
            ...
            $query->where('user_id_1', auth()->id())
                ...
                ->orWhere(function ($roleQuery) {
                    $roleQuery->whereNull('user_id_1')
                        ->whereNull('user_id_2')
                        ->whereRaw('LOWER(target_role) IN (?, ?, ?, ?)', ['admin', 'seller', 'driver', 'customer'])
                        ->whereExists(function ($subQuery) {
                            $subQuery->from('inquiries')
                                ->whereColumn('inquiries.conversation_id', 'conversations.id')
                                ->where(function ($messageQuery) {
                                    $messageQuery->where('inquiries.sender_id', auth()->id())
                                        ->orWhere('inquiries.recipient_id', auth()->id());
                                });
                        });
                });
        })
    ...
    foreach ($conversations as $conversation) {
        ...
        } elseif ($conversation->user_id_2 === auth()->id()) {
...
<div class="overflow-hidden rounded-[28px] border border-gray-200 bg-white shadow-[0_18px_60px_rgba(15,23,42,0.08)]">
        ...
        <div class="flex items-center justify-between gap-3">
            <div class="flex items-center gap-3">
                @if(auth()->user()?->profile_photo_url)
                    <img src="{{ auth()->user()->profile_photo_url }}" alt="{{ $adminName }}" class="h-12 w-12 rounded-full object-cover ring-2 ring-white/30">
                @else
                ...
                @endif
                <div>
                    <p class="text-[10px] uppercase tracking-[0.25em] text-emerald-100">Admin</p>
                    <h3 class="text-lg font-bold">{{ $adminName }}</h3>
                </div>
            </div>
            <span class="rounded-full bg-white/15 px-3 py-1 text-xs font-semibold">Live</span>
        </div>
        ...
        <aside data-contact-index class="border-b border-emerald-100 bg-emerald-50/40 md:border-b-0 md:border-r">
            ...
            <div class="space-y-2 p-3">
                @if(count($contacts) > 0)
                    @foreach($contacts as $contact)
                        <button type="button" data-contact-item data-user-id="{{ $contact['user_id'] ?? '' }}" data-name="{{ $contact['name'] }}" data-email="{{ $contact['email'] ?? '' }}" data-role="{{ $contact['role'] ?? 'Support' }}" data-presence="{{ $contact['status'] ?? 'active' }}" data-away-minutes="{{ $contact['away_minutes'] ?? '' }}" data-score="{{ $contact['score'] ?? 0 }}" data-avatar-image="{{ $contact['profile_photo'] ?? '' }}" data-thread-messages='{{ json_encode($contact['messages'] ?? [], JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) }}' class="{{ $loop->first ? 'bg-white shadow-sm ring-1 ring-emerald-100' : 'bg-transparent hover:bg-white/80' }} flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-left transition">
                            <div class="relative">
                                ...
                                @else
                                    <div class="flex h-11 w-11 items-center justify-center rounded-full bg-gradient-to-br from-emerald-600 to-green-500 text-xs font-bold text-white">
                                        {{ $contact['avatar'] }}
                                    </div>
                                @endif
                    ...
                    @endforeach
                @else
                    <div class="flex min-h-[180px] flex-col items-center justify-center rounded-2xl border border-dashed border-emerald-200 bg-white p-5 text-center text-slate-500">
                        ...
                        <p class="mt-1 text-xs text-slate-500">Messages from customers, sellers, or drivers will appear here.</p>
                    </div>
                @endif
            </div>
        </aside>
        ...
        <main data-chat-panel class="hidden min-h-[360px] flex-col bg-white md:flex">
            <header class="flex items-center justify-between gap-2 border-b border-emerald-100 px-3 py-3 sm:px-4">
                ...
                <div class="flex items-center gap-2 text-slate-500">
                    @php
                        $deleteThreadKey = !empty($activeContact['user_id'])
                            ? ('user:' . $activeContact['user_id'] . ':role:' . strtolower((string)($activeContact['role'] ?? 'admin')))
                            : (!empty($activeContact['email'])
                                ? ('email:' . strtolower((string) $activeContact['email']) . ':role:' . strtolower((string)($activeContact['role'] ?? 'admin')))
                                : ('name:' . strtolower((string)($activeContact['name'] ?? 'support')) . ':role:' . strtolower((string)($activeContact['role'] ?? 'admin'))));
                    @endphp
                    @if(!empty($activeContact['conversation_id']) || !empty($activeContact['user_id']) || !empty($activeContact['email']) || !empty($activeContact['name']))
                        <form method="POST" action="{{ !empty($activeContact['conversation_id']) ? route('inquiries.conversations.destroy', $activeContact['conversation_id']) : '#' }}" data-delete-thread-form data-thread-key="{{ $deleteThreadKey }}" data-has-conversation="{{ !empty($activeContact['conversation_id']) ? 'true' : 'false' }}" onsubmit="return confirm('Delete this conversation?');" class="inline-block">
                            @csrf
                            @method('DELETE')
            ...
            <form data-chat-form data-recipient-id="{{ $activeContact['user_id'] ?? '' }}" data-recipient-name="{{ $activeContact['name'] }}" data-recipient-email="{{ $activeContact['email'] ?? '' }}" data-recipient-role="{{ $activeContact['role'] ?? 'Support' }}" method="POST" action="{{ route('inquiries.store') }}" class="border-t border-gray-200 bg-white p-3 sm:p-4">
                @csrf
                ...
                <div class="flex items-end gap-3 rounded-2xl border border-gray-200 bg-gray-50 p-2">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const ensureInboxContact = function (item) {
            ...
            const role = (item.dataset.role || 'admin').toString();
        ...
        const setRecipient = function (item) {
            ...
            const role = item.dataset.role || form.dataset.recipientRole || 'admin';
            const avatarImage = item.dataset.avatarImage || '';
