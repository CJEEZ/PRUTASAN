        ...
        <section class="space-y-6">
            <div class="flex flex-col gap-4 rounded-2xl bg-emerald-800 p-5 text-white shadow-lg sm:flex-row sm:items-center sm:justify-between sm:p-6">
                ...
                <div class="flex items-center gap-2">
                    <form method="POST" action="{{ route('driver.availability') }}" class="flex min-h-[48px] shrink-0 cursor-pointer items-center justify-between gap-4 rounded-xl bg-white/10 px-4 py-3">
                        @csrf @method('PATCH')
                        <span><span class="block text-xs text-emerald-100">Availability</span><span id="availability-label" class="font-bold">{{ $driverAvailable ? 'Online' : 'Offline' }}</span></span>
                        ...
                        <label for="availability-toggle" class="relative h-7 w-12 cursor-pointer rounded-full bg-emerald-300 transition peer-checked:bg-white after:absolute after:left-1 after:top-1 after:h-5 after:w-5 after:rounded-full after:bg-emerald-700 after:transition peer-checked:after:translate-x-5"></label>
                    </form>
                    <a href="{{ route('driver.profile') }}" class="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-full bg-white text-emerald-800 shadow-sm transition hover:bg-emerald-50" aria-label="Open driver profile" title="Profile">
            ...
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-100"><div class="flex items-center justify-between"><p class="text-sm text-gray-500">Today's earnings</p><i class="fas fa-peso-sign text-emerald-600"></i></div><p class="mt-3 text-2xl font-bold text-gray-900">₱0.00</p><p class="mt-1 text-xs text-gray-500">Start a delivery to earn</p></div>
                <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-100"><div class="flex items-center justify-between"><p class="text-sm text-gray-500">Weekly total</p><i class="fas fa-chart-line text-blue-600"></i></div><p class="mt-3 text-2xl font-bold text-gray-900">₱0.00</p><p class="mt-1 text-xs text-gray-500">This week's earnings</p></div>
                <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-100"><div class="flex items-center justify-between"><p class="text-sm text-gray-500">On-time rate</p><i class="fas fa-stopwatch text-orange-600"></i></div><p class="mt-3 text-2xl font-bold text-gray-900">100%</p><p class="mt-1 text-xs text-gray-500">Keep up the good work</p></div>
                <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-100"><div class="flex items-center justify-between"><p class="text-sm text-gray-500">Customer rating</p><i class="fas fa-star text-amber-500"></i></div><p class="mt-3 text-2xl font-bold text-gray-900">New</p><p class="mt-1 text-xs text-gray-500">Build your rating</p></div>
            </div>
            ...
            <section class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-100 sm:p-6"><div class="flex items-center justify-between"><div><h2 class="text-lg font-bold text-gray-900">Performance metrics</h2><p class="mt-1 text-sm text-gray-500">Your delivery performance will update after your first trip.</p></div><i class="fas fa-chart-bar text-xl text-emerald-600"></i></div><div class="mt-5 grid gap-4 sm:grid-cols-3"><div class="rounded-xl bg-emerald-50 p-4"><p class="text-xs font-semibold uppercase tracking-wide text-emerald-700">Deliveries</p><p class="mt-2 text-2xl font-bold text-emerald-900">0</p></div><div class="rounded-xl bg-blue-50 p-4"><p class="text-xs font-semibold uppercase tracking-wide text-blue-700">Distance</p><p class="mt-2 text-2xl font-bold text-blue-900">0 km</p></div><div class="rounded-xl bg-orange-50 p-4"><p class="text-xs font-semibold uppercase tracking-wide text-orange-700">Volume transported</p><p class="mt-2 text-2xl font-bold text-orange-900">0 kg</p></div></div></section>

            <nav class="grid grid-cols-2 gap-2 rounded-2xl bg-white p-2 shadow-sm ring-1 ring-gray-100 sm:grid-cols-3 lg:grid-cols-6"><a href="{{ route('driver.dashboard') }}" class="rounded-xl bg-emerald-50 px-2 py-3 text-center text-xs font-semibold text-emerald-700"><i class="fas fa-truck mb-1 block text-base"></i>Deliveries</a><a href="{{ route('driver.analytics') }}" class="rounded-xl px-2 py-3 text-center text-xs font-semibold text-gray-500 hover:bg-gray-50"><i class="fas fa-chart-column mb-1 block text-base"></i>Analytics</a><a href="{{ route('driver.schedule') }}" class="rounded-xl px-2 py-3 text-center text-xs font-semibold text-gray-500 hover:bg-gray-50"><i class="fas fa-calendar mb-1 block text-base"></i>Schedule</a><a href="{{ route('driver.history') }}" class="rounded-xl px-2 py-3 text-center text-xs font-semibold text-gray-500 hover:bg-gray-50"><i class="fas fa-clock-rotate-left mb-1 block text-base"></i>History</a><a href="{{ route('driver.messages') }}" class="relative rounded-xl px-2 py-3 text-center text-xs font-semibold text-gray-500 hover:bg-gray-50"><i class="fas fa-message mb-1 block text-base"></i>Messages @if(auth()->user()->notifications()->where('is_read', false)->exists())<span class="absolute right-2 top-2 h-2 w-2 rounded-full bg-orange-500" aria-label="Unread messages"></span>@endif</a><form method="POST" action="{{ route('logout') }}" class="contents">@csrf<button type="submit" class="min-h-[52px] rounded-xl px-2 py-3 text-center text-xs font-semibold text-gray-500 transition hover:bg-red-50 hover:text-red-600"><i class="fas fa-sign-out-alt mb-1 block text-base"></i>Logout</button></form></nav>
        </section>
        <script>
            ...
            <section class="rounded-2xl bg-white p-5 shadow-lg ring-1 ring-gray-100 sm:p-8">
                    ...
                    <div class="grid gap-5 sm:grid-cols-3">
                        @foreach([['license_photo', 'Driver license'], ['or_photo', 'OR photo'], ['cr_photo', 'CR photo']] as [$field, $label])
                            <div>
                                ...
                                <label for="{{ $field }}" class="relative mt-2 flex min-h-[150px] cursor-pointer flex-col items-center justify-center overflow-hidden rounded-xl border-2 border-dashed border-gray-200 bg-gray-50 px-3 text-center transition hover:border-emerald-400 hover:bg-emerald-50">
                                    <img data-preview="{{ $field }}" src="{{ $application && $application->{$field . '_path'} ? Storage::disk('public')->url($application->{$field . '_path'}) : '' }}" alt="{{ $label }} preview" class="absolute inset-0 hidden h-full w-full object-cover">
                                    <span data-placeholder="{{ $field }}" class="relative"><i class="fas fa-camera mb-2 block text-xl text-emerald-600"></i><span class="text-xs text-gray-500">Tap to upload<br>JPG or PNG, max 5MB</span></span>
                                    <input id="{{ $field }}" name="{{ $field }}" type="file" accept="image/jpeg,image/png" {{ $application && $application->{$field . '_path'} ? '' : 'required' }} class="sr-only">
                                </label>
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="mt-5 flex flex-col gap-3 border-t border-gray-100 pt-5 sm:flex-row sm:items-center sm:justify-between">
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="flex min-h-[48px] w-full items-center justify-center px-2 py-3 text-base font-semibold text-gray-700 transition hover:text-red-600 sm:w-auto sm:justify-start"><i class="fas fa-sign-out-alt mr-2 text-xl"></i>Logout</button>
                    </form>
                    @if($application && $application->status === 'pending')
                        <button type="button" disabled class="min-h-[48px] w-full cursor-not-allowed rounded-xl bg-amber-100 px-5 py-3 font-bold text-amber-800 sm:w-auto"><i class="fas fa-clock mr-2"></i>Pending admin review</button>
                    @else
                        <button type="submit" form="driver-application-form" class="min-h-[48px] w-full rounded-xl bg-emerald-700 px-5 py-3 font-bold text-white transition hover:bg-emerald-800 focus:outline-none focus:ring-4 focus:ring-emerald-100 sm:w-auto">{{ $application && $application->status === 'rejected' ? 'Resubmit for admin review' : 'Submit for admin review' }} <i class="fas fa-arrow-right ml-2"></i></button>
                    @endif
                </div>
            </section>
        </div>
