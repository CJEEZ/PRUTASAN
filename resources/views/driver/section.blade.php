...
<main class="min-h-screen px-3 py-6 sm:px-6 lg:px-8">
    <div class="mx-auto max-w-5xl">
        ...
        @if($section === 'analytics')
            ...
            @endphp
            <div class="driver-analytics-metrics flex gap-2 overflow-x-auto pb-1 sm:grid sm:grid-cols-3 sm:overflow-visible">
                <div class="min-w-[145px] shrink-0 rounded-xl bg-white p-3 shadow-sm ring-1 ring-gray-100 sm:min-w-0"><p class="text-xs text-gray-500">Completed deliveries</p><p class="mt-1 text-2xl font-bold text-emerald-700">{{ $completed }}</p><p class="mt-0.5 text-[10px] text-gray-400">All time</p></div>
                <div class="min-w-[145px] shrink-0 rounded-xl bg-white p-3 shadow-sm ring-1 ring-gray-100 sm:min-w-0"><p class="text-xs text-gray-500">Active assignments</p><p class="mt-1 text-2xl font-bold text-blue-700">{{ $active }}</p><p class="mt-0.5 text-[10px] text-gray-400">Currently assigned</p></div>
                <div class="min-w-[145px] shrink-0 rounded-xl bg-white p-3 shadow-sm ring-1 ring-gray-100 sm:min-w-0"><p class="text-xs text-gray-500">This month</p><p class="mt-1 text-2xl font-bold text-orange-700">{{ $deliveredThisMonth }}</p><p class="mt-0.5 text-[10px] text-gray-400">Completed trips</p></div>
            </div>
        ...
        <nav class="driver-navigation sticky bottom-0 z-30 mt-6 flex gap-1.5 overflow-x-auto rounded-xl bg-white/95 p-1.5 shadow-[0_-6px_20px_rgba(15,23,42,0.08)] ring-1 ring-gray-100 backdrop-blur lg:static lg:grid lg:grid-cols-6 lg:overflow-visible lg:bg-white lg:shadow-sm"><a href="{{ route('driver.dashboard') }}" class="min-w-[76px] shrink-0 rounded-lg px-2 py-2 text-center text-[10px] font-semibold {{ $section === 'dashboard' ? 'bg-emerald-50 text-emerald-700' : 'text-gray-500 hover:bg-gray-50' }} lg:min-w-0"><i class="fas fa-truck mb-0.5 block text-sm"></i>Deliveries</a><a href="{{ route('driver.analytics') }}" class="min-w-[76px] shrink-0 rounded-lg px-2 py-2 text-center text-[10px] font-semibold {{ $section === 'analytics' ? 'bg-emerald-50 text-emerald-700' : 'text-gray-500 hover:bg-gray-50' }} lg:min-w-0"><i class="fas fa-chart-column mb-0.5 block text-sm"></i>Analytics</a><a href="{{ route('driver.schedule') }}" class="min-w-[76px] shrink-0 rounded-lg px-2 py-2 text-center text-[10px] font-semibold {{ $section === 'schedule' ? 'bg-emerald-50 text-emerald-700' : 'text-gray-500 hover:bg-gray-50' }} lg:min-w-0"><i class="fas fa-calendar mb-0.5 block text-sm"></i>Schedule</a><a href="{{ route('driver.history') }}" class="min-w-[76px] shrink-0 rounded-lg px-2 py-2 text-center text-[10px] font-semibold {{ $section === 'history' ? 'bg-emerald-50 text-emerald-700' : 'text-gray-500 hover:bg-gray-50' }} lg:min-w-0"><i class="fas fa-clock-rotate-left mb-0.5 block text-sm"></i>History</a><a href="{{ route('driver.messages') }}" class="min-w-[76px] shrink-0 rounded-lg px-2 py-2 text-center text-[10px] font-semibold text-gray-500 hover:bg-gray-50 lg:min-w-0"><i class="fas fa-message mb-0.5 block text-sm"></i>Messages</a><form method="POST" action="{{ route('logout') }}" class="contents"><button type="submit" class="min-h-[44px] min-w-[76px] shrink-0 rounded-lg px-2 py-2 text-center text-[10px] font-semibold text-gray-500 transition hover:bg-red-50 hover:text-red-600 lg:min-w-0"><i class="fas fa-sign-out-alt mb-0.5 block text-sm"></i>Logout</button></form></nav>
    </div>
</main>
</body>
</html>
