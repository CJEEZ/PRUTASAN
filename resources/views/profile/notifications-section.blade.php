@php
    ...
    // Get conversations where the current user is one of the participants
    $conversations = \App\Models\Conversation::query()
        ->where(function ($query) use ($currentUser) {
            ...
            $allowedCustomerRoles = ['customer', 'admin', 'driver'];

            $query->orWhere(function ($roleQuery) use ($currentUser, $allowedCustomerRoles) {
                $roleQuery->whereNull('user_id_1')
                    ->whereNull('user_id_2')
                    ->whereRaw('LOWER(target_role) IN (?, ?, ?)', $allowedCustomerRoles)
                    ->whereExists(function ($subQuery) use ($currentUser) {
                        $subQuery->from('inquiries')
                            ...
                            ->where(function ($messageQuery) use ($currentUser) {
                                $messageQuery->where('inquiries.sender_id', $currentUser?->id)
                                    ->orWhere('inquiries.recipient_id', $currentUser?->id);
                            });
                    });
            });

            if ($currentUser) {
                $query->orWhereExists(function ($subQuery) use ($currentUser) {
                    $subQuery->from('inquiries')
                        ->whereColumn('inquiries.conversation_id', 'conversations.id')
                        ->where(function ($messageQuery) use ($currentUser) {
                            $messageQuery->where('inquiries.sender_id', $currentUser->id)
    ...
    foreach ($conversations as $conversation) {
        ...
        if ($currentUser && $conversation->user_id_1 === $currentUser->id) {
            $peer = $conversation->user2;
            $peerEmail = $conversation->user2?->email;
        } elseif ($currentUser && $conversation->user_id_2 === $currentUser->id) {
        ...
        $allowedCustomerRoles = ['customer', 'admin', 'driver'];
        if (!in_array($peerRole, $allowedCustomerRoles, true) && !in_array($conversationRole, $allowedCustomerRoles, true)) {
            continue;
        }

        $key = ($currentUser->id ?? null)
            ? ('user:' . $currentUser->id . ':peer:' . ($peer->id ?? 0))
    ...
    foreach ($profileUsers as $profileUser) {
        ...
        $profileUserKey = ($currentUser->id ?? null)
            ? ('user:' . $currentUser->id . ':peer:' . $profileUser->id)
        ...
        <main class="flex min-h-[360px] flex-col bg-white">
            <header class="flex items-center justify-between gap-3 border-b border-emerald-100 px-3 py-3 sm:px-4">
                ...
                <div class="flex items-center gap-2 text-slate-500">
                    @if(!empty($activeContact['conversation_id']) || !empty($activeContact['messages']))
                        @php
                            $profileThreadKey = $activeContact['user_id']
                                ? 'user:' . $activeContact['user_id'] . ':role:' . strtolower((string) ($activeContact['role'] ?? 'customer'))
                                : ($activeContact['email']
                                    ? 'email:' . strtolower((string) $activeContact['email']) . ':role:' . strtolower((string) ($activeContact['role'] ?? 'customer'))
                                    : ('name:' . strtolower((string) ($activeContact['name'] ?? 'support team')) . ':role:' . strtolower((string) ($activeContact['role'] ?? 'customer'))));
                        @endphp
            ...
            <div data-chat-thread class="h-[280px] min-h-[200px] max-h-[50vh] space-y-3 overflow-y-auto bg-[radial-gradient(circle_at_top,_rgba(16,185,129,0.08),_transparent_30%)] p-3 sm:h-[320px] sm:max-h-[55vh] sm:p-4">
                ...
                <div data-chat-typing class="hidden flex justify-start">
                    <div class="rounded-2xl bg-slate-100 px-4 py-3 text-sm text-slate-500 shadow-sm">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const pruneStaleThreadCache = function () {
            ...
            Array.from(document.querySelectorAll('[data-contact-item]')).forEach(function (item) {
                ...
                const role = item.dataset.role || 'customer';
        ...
        const ensureInboxContact = function (item) {
            ...
            const role = (item.dataset.role || 'customer').toString();
        ...
        const setRecipient = function (item) {
            ...
            const role = item.dataset.role || form.dataset.recipientRole || 'customer';
            const avatarImage = item.dataset.avatarImage || '';
