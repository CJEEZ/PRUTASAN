...
@php
    ...
    // Get conversations where the seller is one of the participants
    $conversations = \App\Models\Conversation::query()
        ->where(function ($query) use ($seller) {
            if ($seller) {
                $query->where('user_id_1', $seller->id)
                    ->orWhere('user_id_2', $seller->id);
            }

            $allowedSellerRoles = ['seller', 'admin'];

            $query->orWhere(function ($roleQuery) use ($seller, $allowedSellerRoles) {
                $roleQuery->whereNull('user_id_1')
                    ->whereNull('user_id_2')
                    ->whereRaw('LOWER(target_role) IN (?, ?)', $allowedSellerRoles)
                    ->whereExists(function ($subQuery) use ($seller) {
                        $subQuery->from('inquiries')
                            ->whereColumn('inquiries.conversation_id', 'conversations.id')
                            ->where(function ($messageQuery) use ($seller) {
                                $messageQuery->where('inquiries.sender_id', $seller?->id)
                                    ->orWhere('inquiries.recipient_id', $seller?->id);
                            });
                    });
            });

            if ($seller) {
                $query->orWhereExists(function ($subQuery) use ($seller) {
                    $subQuery->from('inquiries')
                        ->whereColumn('inquiries.conversation_id', 'conversations.id')
                        ->where(function ($messageQuery) use ($seller) {
                            $messageQuery->where('inquiries.sender_id', $seller->id)
    ...
    foreach ($conversations as $conversation) {
        ...
        if ($seller && $conversation->user_id_1 === $seller->id) {
            $peer = $conversation->user2;
            $peerEmail = $conversation->user2?->email;
        } elseif ($seller && $conversation->user_id_2 === $seller->id) {
        ...
        $allowedSellerRoles = ['seller', 'admin'];
        if (!in_array($peerRole, $allowedSellerRoles, true) && !in_array($conversationRole, $allowedSellerRoles, true)) {
            continue;
        }

        $key = ($seller->id ?? null)
            ? ('user:' . $seller->id . ':peer:' . ($peer->id ?? 0))
        ...
        foreach ($messages as $msg) {
            $isOutgoing = $msg->sender_id === $seller?->id;
    ...
    foreach ($profileUsers as $profileUser) {
        ...
        $profileUserKey = ($seller->id ?? null)
            ? ('user:' . $seller->id . ':peer:' . $profileUser->id)
            : ('profile:' . $profileUser->id);

        if (isset($contactMap[$profileUserKey])) {
            continue;
        }

        $contactMap[$profileUserKey] = [
...
<div class="flex w-full flex-col gap-6 p-3 sm:p-4 lg:flex-row lg:gap-6 lg:p-6" style="min-height:70vh;">
    <aside class="hidden w-64 rounded-xl border border-gray-200 bg-white p-4 shadow-sm lg:block">
    ...
    <div class="flex-1">
        <div class="overflow-hidden rounded-[28px] border border-emerald-100 bg-white shadow-[0_18px_60px_rgba(15,23,42,0.08)]">
            <div class="border-b border-emerald-100 bg-gradient-to-r from-emerald-700 via-emerald-600 to-green-500 p-4 text-white sm:p-5">
                <div class="flex items-center justify-between gap-3">
                    <div class="flex items-center gap-3">
                        @if($seller?->profile_photo_url)
                            <img src="{{ $seller->profile_photo_url }}" alt="{{ $sellerName }}" class="h-12 w-12 rounded-full object-cover ring-2 ring-white/30">
                        @else
                        ...
                        @endif
                        <div>
                            <p class="text-[10px] uppercase tracking-[0.25em] text-emerald-100">Seller</p>
                            <h3 class="text-lg font-bold">{{ $sellerName }}</h3>
                        </div>
                    </div>
                ...
                <main class="flex min-h-[500px] flex-col bg-white">
                    <header class="flex items-center justify-between gap-3 border-b border-emerald-100 px-4 py-4 sm:px-5">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h4 id="seller-profile-name" data-testid="seller-profile-name" class="font-semibold text-slate-900">{{ $activeContact['name'] }}</h4>
                        ...
                        <div class="flex items-center gap-2 text-slate-500">
                            @php
                                $sellerThreadKey = !empty($activeContact['user_id'])
                                    ? 'user:' . $activeContact['user_id'] . ':role:' . strtolower((string) ($activeContact['role'] ?? 'seller'))
                                    : (!empty($activeContact['email'])
                                        ? 'email:' . strtolower((string) $activeContact['email']) . ':role:' . strtolower((string) ($activeContact['role'] ?? 'seller'))
                                        : ('name:' . strtolower((string) ($activeContact['name'] ?? 'support team')) . ':role:' . strtolower((string) ($activeContact['role'] ?? 'seller'))));
                            @endphp
                            @if(!empty($activeContact['conversation_id']) || !empty($activeContact['user_id']) || !empty($activeContact['email']) || !empty($activeContact['name']))
                                <form method="POST" action="{{ !empty($activeContact['conversation_id']) ? route('inquiries.conversations.destroy', $activeContact['conversation_id']) : '#' }}" data-delete-thread-form data-thread-key="{{ $sellerThreadKey }}" data-has-conversation="{{ !empty($activeContact['conversation_id']) ? 'true' : 'false' }}" data-legacy-thread-key="{{ $activeContact['user_id'] ?? $activeContact['email'] ?? $activeContact['name'] ?? '' }}" onsubmit="return confirm('Delete this conversation?');" class="inline-block">
                                    @csrf
                                    @method('DELETE')
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const nameEl = document.querySelector('#seller-profile-name');
        ...
        const pruneStaleThreadCache = function () {
            ...
            Array.from(document.querySelectorAll('[data-contact-item]')).forEach(function (item) {
                ...
                const role = item.dataset.role || 'seller';
        ...
        const refreshPresence = function (item) {
            ...
            fetch('{{ route('presence.show', ['user' => '__USER__']) }}'.replace('__USER__', userId), { headers: { 'Accept': 'application/json' } })
                ...
                .then(function (presence) {
                    ...
                    if (item.classList.contains('ring-1')) {
                        const status = document.querySelector('[data-profile-status]');
                        if (status) status.textContent = item.dataset.role + ' • ' + (presence.status === 'online' ? 'Online now' : (presence.away_minutes ? 'Away for ' + presence.away_minutes + ' min' : 'Away'));
                    }
                })
                .catch(function () {});
        };
        ...
        const ensureInboxContact = function (item) {
            ...
            const role = (item.dataset.role || 'seller').toString();
        ...
        const setRecipient = function (item) {
            ...
            const role = item.dataset.role || form.dataset.recipientRole || 'seller';
            const avatarImage = item.dataset.avatarImage || '';
            const threadKey = userId
                ? 'user:' + String(userId) + ':role:' + String(role).trim().toLowerCase()
                : (email
                    ? 'email:' + String(email).trim().toLowerCase() + ':role:' + String(role).trim().toLowerCase()
                    : (name
                        ? 'name:' + String(name).trim().toLowerCase() + ':role:' + String(role).trim().toLowerCase()
        ...
        if (form && input && thread) {
            ...
            form.addEventListener('submit', function (event) {
                ...
                const threadKey = form.dataset.threadKey || form.dataset.recipientId || form.querySelector('input[name="recipient_id"]').value || '';
                const currentThread = threadKey && threadCache[threadKey] ? threadCache[threadKey] : [];
                formData.set('message', value);
                formData.set('recipient_id', form.dataset.recipientId || form.querySelector('input[name="recipient_id"]').value || '');
                formData.set('recipient_name', form.dataset.recipientName || form.querySelector('input[name="recipient_name"]').value || 'Support team');
                formData.set('recipient_email', form.dataset.recipientEmail || form.querySelector('input[name="recipient_email"]').value || '');
                formData.set('recipient_role', form.dataset.recipientRole || form.querySelector('input[name="recipient_role"]').value || 'Support');
                formData.set('target_role', formData.get('recipient_role'));
